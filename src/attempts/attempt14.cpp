...
struct bitset {
    ...
    bitset emptied() const{
        ...
        empty_copy.pos_bits.clear();
        empty_copy.neg_bits.clear();
        return empty_copy;
    }
...
struct cdcl {
    ...
    std::pair<bitset, bitset> resolve_conflict(const bitset &conflict_clause, const std::vector<bitset> *clauses) {
        ...
        // variables in conflict clause and the decision level
        bitset variables = learned_clause & variables_at_decision_level;

        // resolve conflict
        while (variables.num_variables() > 1) {

            // latest assigned variable in decision level
            for (auto it = trail->rbegin(); it != trail->rend(); ++it) {
                if (it->second == -1) break; // stop at decision variables
                if (variables.test_or(it->first)) {
                    bit latest_variable = it->first;
                    int reason_clause_index = it->second;

                    // resolve using reason clause
                    bitset reason_clause = (*clauses)[reason_clause_index];
                    learned_clause.set(reason_clause);
                    learned_clause.remove(latest_variable);

                    // update variables
                    variables = learned_clause & variables_at_decision_level;
        ...
        // backtrack
        bit highest_decision_variable(0);
        bitset backtrack_variables = conflict_clause.emptied();
        int i = learned_clause.num_variables()-1;
        for (auto it = trail->rbegin(); it != trail->rend(); ++it) {
            if (i == 0) { // stop before highest decision variable
                trail->resize(trail->size() - std::distance(trail->rbegin(), it)); // cutoff the trail
                break; 
            }
        ...
        return {learned_clause.neg(), backtrack_variables};
    }
...
struct sat_solver {
    ...
    //     // luby restarts
    //     bool luby = true;
    //     if (luby && iteration_count >= max_iterations) return "stopped";
    ...
    //     // random polarity of variable
    //     std::uniform_int_distribution<> dis(0, 1);
    //     int polarity = dis(gen) * 2 - 1;
        
    //     // branching
    //     bitset assignment_1(assignment);
    //     bit unassigned_bit_1(polarity*unassigned_variable);
    //     sat_data.emplace(unassigned_bit_1);
    //     std::string result_1 = solve(assignment_1);
    //     if (result_1 == "sat") {
    //         assignment = assignment_1;
    //         return "sat";
    //     } else if (result_1 == "stopped") return "stopped";

    //     // visitation
    //     bitset visited_assingment_1(assignment);
    //     visited_assingment_1.set(polarity*unassigned_variable);
    //     if (visitations_toggle) visited.insert(visited_assingment_1);

    //     // luby restarts
    //     if (luby) iteration_count++;

    //     // branching on opposite polarity
    //     sat_data.clear_queue(); // clear queue
    //     bitset assignment_2(assignment);
    //     bit unassigned_bit_2(-1*polarity*unassigned_variable);
    //     sat_data.emplace(unassigned_bit_2);
    //     std::string result_2 = solve(assignment_2);
    //     if (result_2 == "sat") {
    //         assignment = assignment_2;
    //         return "sat";
    //     } else if (result_2 == "stopped") return "stopped";

    //     // visitation slight improvement
    ...
    std::optional<bitset> unit_propagation(bitset &assignment) {
        ...
        while (sat_data.unit_queue_size() > 0) {
            ...
            const bit &bit_num = sat_data.pop();
            const bit &bit_num_neg = bit_num.neg();
            if (assignment.test(bit_num_neg)) throw std::runtime_error("attempted to assign negative of variable"); // early stop
    ...
    std::string solve(bitset &assignment) {
        std::cout << "starting cdcl solving" << std::endl;

        // settings
        // bool visitations_toggle = true;

        // debugging -- stop iterations
        int count = 0;
        ...
        while (true) {
            ...
            if (conflict) { // conflict found
                ...
                // resolve conflict (learned clause, backtrack variables)
                std::pair<bitset,bitset> backtrack_result = backtrack.resolve_conflict(conflict.value(), sat_data.clauses);
                if (backtrack_result.second.num_variables() == 0) return "unsat";
                assignment &= backtrack_result.second.neg();
                sat_data.add_clause(backtrack_result.first);
            }
            ...
            bitset assignment_1(assignment);
            bit unassigned_bit_1(polarity*unassigned_variable);
            backtrack.decision(unassigned_bit_1);
            sat_data.emplace(unassigned_bit_1);
        }
        return "unsat";
    }
    ...
    std::string restart(bitset &assignment) {
        ...
        // notes:
            ...
            // cdcl
            // remove the recursion from the solver

            // track memeory usage and other statistics...

        // C1597_024 -> new target
            // with restarts: 924.338
            // w/o restarts: N/A, does not solve in a reasonable time

        // we lost U50_4450_035, but gained others
            // maybe has to do with luby restarts and not...
            // 651.12s with luby restarts
            // used to be ~60s
...
int main(int argc, char* argv[]) {
    ...
    // formatted_result = result; // override...
