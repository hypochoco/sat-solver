...
// at this point, it's helpful to make the sat into a class for luby restarts and randomization
...
std::string solve(
    ...
    // instantiating this is probably slow
        // make a class

    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> dis(0, unassigned_variables.size() - 1);
    int unassigned_variable = unassigned_variables[dis(gen)];

    // stopping condition
    if (unassigned_variable == -1) {
