...
// before testing random restarts with branching strategy
...
struct pool {
    ...
    std::vector<std::vector<int>> create_branches(const int &n) {
        ...
        // note
            // smarter multithreading strategy doesn't seem like it'll help
                // branches generally don't finish before each other

            // better branching heuristic?
            // faster / more memory efficient code?
            // cdcl -> seemed pretty close, but not there -> also seemed slow
            // random thread restarts
                // thinking of the tree..
                // after some iterations, stop, pick something random from the stack
                    // add whatever you were looking at back onto the stack
                // base restarts on luby restarts
